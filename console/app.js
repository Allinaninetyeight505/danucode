...
(function () {
  ...
  function renderFileTree(tab) {
    ...
    for (const [path, info] of entries) {
      ...
      html.push(`<div class="file-item${cls}" title="${esc(path)}" data-path="${esc(path)}">${indent}<span class="icon">&#128196;</span><span class="name">${esc(fileName)}</span></div>`);
    }

    dom.fileTree.innerHTML = html.join('');

    // Right-click context menu on file items
    dom.fileTree.querySelectorAll('.file-item[data-path]').forEach(item => {
      item.addEventListener('contextmenu', (e) => {
        e.preventDefault();
        showFileContextMenu(e.clientX, e.clientY, item.dataset.path);
      });
    });
  }
  ...
  function showFileContextMenu(x, y, filePath) {
    ...
    menu.style.top = y + 'px';
    menu.innerHTML = `
      <div class="context-menu-item" data-action="copy">Copy path</div>
      <div class="context-menu-item" data-action="paste">Paste into input</div>
    `;
  ...
  function createActivityElement(event) {
    switch (event.type) {
      ...
      case 'user': return createUserElement(event);
      default: return null;
    }
  }
  ...
  function createUserElement(event) {
    const div = document.createElement('div');
    div.className = 'activity-item type-user';
    div.dataset.category = 'response';
    div.innerHTML = `<span class="activity-badge user">you</span><div class="activity-content">${esc(event.content)}</div>`;
    return div;
  }
  ...
  async function browseDir(dirPath) {
    ...
    try {
      ...
      if (data.parent) {
        ...
        parentEl.innerHTML = '<span class="browse-icon">..</span> parent directory';
        parentEl.addEventListener('click', () => browseDir(data.parent));
      ...
      currentEl.className = 'browse-item browse-current';
      currentEl.textContent = currentName + ' (current)';
      browser.appendChild(currentEl);

      // Subdirectories
      for (const dir of data.dirs) {
        const fullPath = data.current + (data.current.endsWith('\\') || data.current.endsWith('/') ? '' : '\\') + dir;
        const dirEl = document.createElement('div');
        dirEl.className = 'browse-item';
        dirEl.innerHTML = `<span class="browse-icon">&#128193;</span> `;
        const nameSpan = document.createElement('span');
        nameSpan.textContent = dir;
        dirEl.appendChild(nameSpan);
        dirEl.addEventListener('click', () => browseDir(fullPath));
        browser.appendChild(dirEl);
      }

      if (data.dirs.length === 0) {
        const emptyEl = document.createElement('div');
        emptyEl.style.cssText = 'padding:8px 12px;color:var(--text-muted);font-style:italic';
